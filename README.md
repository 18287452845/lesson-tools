...
- **数量固定** - 最终教案数量始终由总课时 ÷ 每份教案课时决定，章节仅作参考
- **灵活分组** - 可配置每份教案课时数（默认2课时/教案）
- **章节缓存** - 生成的章节模板自动缓存，数量不匹配时自动清理并重新生成
- **多种输入方式** - 可选输入章节参考，AI 按课时重新划分
- **实时进度** - 后台生成，实时查看进度和状态
...
- **每份教案课时**: 默认 2 课时（可调整）
- **章节来源**:
  - **AI 自动生成**: AI 根据课程信息与课时自动划分知识点
  - **手动输入**: 可输入章节标题作为参考，AI 会按总课时重新划分
  - **使用已有模板**: 复用历史章节作为参考，AI 会按总课时重新划分
