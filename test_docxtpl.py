"""Simple test to verify docxtpl works correctly with our template."""
...
test_data = {
    ...
    "teaching_steps": [
        ...
        },
        {
            "stage": "课堂练习",
            "duration": "20分钟",
            "teacher_activity": "指导学生完成安全分析练习",
            "student_activity": "分组完成练习，讨论解决方案",
            "design_intent": "巩固所学知识"
        },
        {
            "stage": "总结反馈",
            "duration": "5分钟",
            "teacher_activity": "总结本节课重点，布置作业",
            "student_activity": "记录作业要求，提出疑问",
            "design_intent": "梳理知识体系"
        }
    ],
    ...
    "reflection": "本次课程学生参与度高，对实际案例很感兴趣。下次可以增加更多互动环节。",
}

print("🧪 Testing docxtpl with Yunnan Forestry College Template\n")
print("="*80)

# Template path
template_path = "storage/templates/yunnan_forestry_college_template.docx"
output_path = "storage/outputs/test_docxtpl_render.docx"
...
try:
    # Load template
    print("🔄 Loading template with docxtpl...")
    doc_template = DocxTemplate(template_path)
    
    # Render
    print("🔄 Rendering document with test data...")
    doc_template.render(test_data)
    ...
    if output_doc.paragraphs:
        for i, para in enumerate(output_doc.paragraphs[:5]):
    ...
    print(f"\n👉 Please check the output file: {output_path}")
